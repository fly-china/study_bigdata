...
//
// Related Topics 贪心 数组 双指针 排序 👍 892 👎 0
...
public class AssignCookies {
    ...
    class Solution {
        ...
        public int findContentChildren(int[] g, int[] s) {
            ...
            int meetChildNums = 0; // 满足小孩的数量
            int childIdx = 0; // 遍历的小孩 idx

            // 从小到大遍历饼干，按照最小满足原则分配给小孩。（小饼干先喂饱小胃口）
            for (int i = 0; i < s.length && childIdx < g.length; i++) {
                if (s[i] >= g[childIdx]) { // 满足胃口
                    childIdx++;
                    meetChildNums++;
                }
            }

            return meetChildNums;
        }
    }
